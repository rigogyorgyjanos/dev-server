...
bool CClientManager::InitializeRefineTable()
{
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		// NOTE: material_count must default to REFINE_MATERIAL_MAX_NUM before the loop -
		// if a recipe fills every material slot (no zero vnum to break on), the loop below
		// never assigns material_count, and it would otherwise stay at the memset'd 0,
		// silently turning a fully-loaded recipe into a "0 materials required" recipe.
...
bool CClientManager::InitializeShopEXTable()
{
	...
	char s_szQuery[1024];
	int len = sprintf(s_szQuery, "SELECT shopex.vnum, shopex.name, shopex.npc_vnum, "
										"shopex_item.item_vnum, shopex_item.count, shopex_item.price, shopex_item.price_type+0, "
	...
	while ((data = mysql_fetch_row(pRes->pSQLResult)))
	{
		...
		// itemmel vasarlas
		for(auto i = 0; i < 5; ++i){
			str_to_number(pItem->price_items[i].vnum, data[col++]);
			str_to_number(pItem->price_items[i].count, data[col++]);
		}
	...
	for (itertype(m_vec_itemTable) it = m_vec_itemTable.begin(); it != m_vec_itemTable.end(); it++)
	{
		if (g_stLocaleNameColumn != "name")
		{
			...
			snprintf(query, sizeof(query),
				...
				"applytype3, applyvalue3, applytype4, applyvalue4, applytype5, applyvalue5, "
				...
				t.aApplies[3].bType, t.aApplies[3].lValue, t.aApplies[4].bType, t.aApplies[4].lValue, t.aApplies[5].bType, t.aApplies[5].lValue,
