...
#ifdef __FARM_SESSION_SYSTEM__
		{
			BYTE bFarmSessionCategory = IsStone() ? FARM_SESSION_MOB_STONE : (GetMobRank() >= MOB_RANK_KING ? FARM_SESSION_MOB_BOSS : FARM_SESSION_MOB_NORMAL);
			CFarmSessionManager::instance().OnKill(pkAttacker, GetRaceNum(), bFarmSessionCategory);
		}
#endif
