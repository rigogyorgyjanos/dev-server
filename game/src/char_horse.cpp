...
bool CHARACTER::StartRiding()
{
	if (IsDead() == true)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("쓰러진 상태에서는 말에 Ż 수 없습니다."));
		return false;
	}
	
	if (IsPolymorphed())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("변신 상태에서는 말에 Ż 수 없습니다."));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("예복을 입은 상태에서 말을 Ż 수 없습니다."));
		return false;
	}
	...
	if (false == CHorseRider::StartRiding())
	{
		if (GetHorseLevel() <= 0)
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("말의 스테미너가 부족하여 말을 Ż 수 없습니다."));
