...
ACMD(do_user_horse_feed)
{
	...
	if (ch->GetHorse() == NULL)
	{
		if (ch->IsHorseRiding() == false)
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("말을 ź 상태에서는 먹이를 줄 수 없습니다."));
		return;
	}
...
extern void DoHotRestart_Game();

EVENTINFO(hotrestart_event_data)
{
	int seconds;

	hotrestart_event_data()
	: seconds( 0 )
	{
	}
};

EVENTFUNC(hotrestart_event)
{
	hotrestart_event_data* info = dynamic_cast<hotrestart_event_data*>( event->info );

	if ( info == NULL )
	{
		sys_err( "hotrestart_event> <Factor> Null pointer" );
		return 0;
	}
	...
	{
		char buf[64];
		snprintf(buf, sizeof(buf), LC_TEXT("A szerver %d masodperc mulva ujraindul."), info->seconds);
		SendNotice(buf);
	...
	SendNotice(LC_TEXT("A szerver ujraindul, kerlek csatlakozz vissza nehany masodperc mulva."));
	...
	if (db_clientdesc)
	{
		db_clientdesc->DBPacket(HEADER_GD_HOTRESTART, 0, NULL, 0);
		// DBPacket() only queues bytes into the output buffer - the actual send normally
		// waits for the next io_loop()/ProcessOutput() pass. We're about to close sockets
		// and execve() within this same event tick (no further io_loop pass will happen),
		// so force a synchronous, blocking flush now or db would never see this packet.
...
void HotRestart(int iSec)
{
	hotrestart_event_data* info = AllocEventInfo<hotrestart_event_data>();
	info->seconds = iSec;

	event_create(hotrestart_event, info, 1);
}
...
ACMD(do_mount)
{
	...
	{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("거기에는 Ż 수 없어요."));
	return;
	}
...
ACMD(do_restart)
{
	...
	//PREVENT_HACK
	//DESC : 창고, 교환 â 후 포탈을 사용하는 버그에 이용될수 있어서
	//		쿨타임을 추가 
	if (subcmd == SCMD_RESTART_TOWN)
	{
		if (ch->IsHack())
		{
			//길드맵, 성지맵에서는 üũ 하지 않는다.
...
static const char* FN_point_string(int apply_number)
{
	switch (apply_number)
	{
		...
		case POINT_DODGE:			return LC_TEXT("Ȱ 공격 회피 확률 %d%%");
