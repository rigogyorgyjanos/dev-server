...
// item socket 이 프로토타입과 같은지 üũ -- by mhh
...
bool CHARACTER::UseItem(TItemPos Cell, TItemPos DestCell)
{
	...
	if (IS_SUMMON_ITEM(item->GetVnum()))
	{
		...
		// 경혼반지 사용지 상대방이 SUMMONABLE_ZONE에 있는가는 WarpToPC()에서 üũ
...
bool CHARACTER::DestroyItem(TItemPos Cell)
{
    ...
    if (!CanHandleItem())
    {
        if (NULL != DragonSoul_RefineWindow_GetOpener())
            ChatPacket(CHAT_TYPE_INFO, LC_TEXT("��*E*A��A�� ���� ��oAA������*��A ����AIAUA�� ��A���� ��o ������A��I��U."));
        return false;
    }
    if (IsDead())
        return false;
...
#ifdef ENABLE_SWITCHBOT
		else if (DestCell.IsSwitchbotPosition() && !SwitchbotHelper::IsValidItem(item))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Invalid item type for switchbot."));
			return false;
		}
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	// 무언가를 ź 상태에서 턱시도 입기 금지
	if (iWearCell == WEAR_BODY && IsRiding() && (item->GetVnum() >= 11901 && item->GetVnum() <= 11904))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("말을 ź 상태에서 예복을 입을 수 없습니다."));
		return false;
	}
